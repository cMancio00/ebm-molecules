...
class Novelty(Metric):
    """
    The percentage of novel molecules generated by the model.
    A molecule is considered novel if it is unique in the training dataset.
    """
    ...
    def update(self, generated_batch: DenseData, training_smiles: Set[str], batch_size: int) -> None:
        if not isinstance(generated_batch, DenseData):
            raise ValueError("Can only compute novelty metrics on DenseData")
        ...
        self.novel += len(smiles.difference(training_smiles))
        self.total += batch_size
